...
export class CreateViewMapeamentoDadosMpesSelo1771506800000 implements MigrationInterface {

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            CREATE OR REPLACE VIEW public.vw_mapeamento_dados_mpes_selo
            AS WITH base AS (
                    SELECT mc.cnpj,
                        ...
                        c.size,
                        c.cpe_return,
                        c.created_at AS criacao_empresa
                    FROM vw_monitoramento_de_cadastrados mc
                        JOIN "biud-esg".companies c ON c.cnpj::text = mc.cnpj::text
                        LEFT JOIN "biud-esg".user_companies uc ON c.id::text = uc.company_id::text
                    ...
                    SELECT question_documents.survey_id,
                        min(question_documents.uploaded_at) AS inicio_diagnostico,
                        max(question_documents.uploaded_at) AS fim_diagnostico
                    FROM "biud-esg".question_documents
                    GROUP BY question_documents.survey_id
                ), best_answer AS (
                    ...
                    FROM (
                            SELECT s.id AS survey_id,
                                q_1.id AS question_id,
                                ao.option::text AS resposta,
                                3 AS prio
                            FROM survey_latest s
                                JOIN "biud-esg".answer_options ao ON ao.survey_id = s.id
                                    AND ao.selected
                                JOIN "biud-esg".questions q_1 ON q_1.id = ao.question_id
                                    AND q_1.tag::text <> 'diagnosis'::text
                            UNION ALL
                            SELECT s.id,
                                q_1.id,
                                CASE
                            ...
                            FROM survey_latest s
                                ...
                                JOIN "biud-esg".questions q_1 ON q_1.id = a.question_id
                                    AND q_1.tag::text <> 'diagnosis'::text
                            UNION ALL
                            ...
                            FROM survey_latest s
                                ...
                                JOIN "biud-esg".questions q_1 ON q_1.tag::text = 'diagnosis'::text
                                    AND q_1.number = unp.num
                        ) x
                    ORDER BY x.survey_id,
                        x.question_id,
                        x.prio DESC
                )
            SELECT b_base.cnpj,
                ...
                b_base.criacao_empresa::timestamp(0) without time zone AS criacao_empresa,
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`DROP VIEW IF EXISTS public.vw_mapeamento_dados_mpes_selo;`);
    }
}
